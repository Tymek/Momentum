...
export type ShadowType = (
  depth: number,
  options?: { color?: string; type?: keyof typeof depthMap },
) => {
  shadowColor: string
  shadowOffset: {
    width: 0
...
const shadow: ShadowType = (depth, options = {}) => {
  const { color, type } = { color: '#000', type: 'penumbra', ...options }
  const raw = depthMap[type as keyof typeof depthMap][depth] || '0px 3px 4px 0px'
